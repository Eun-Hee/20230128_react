...
const getObj = (name, age) => ({ name: name, age: age }); // ()를 제외하면 코드 블록으로 인식하여 오류가 발생함. 객체를 return할 때는 ()로 감싸준다.

sum(10);

/*
    매개변수 rest
        - 매개변수가 몇개일지 정해지지 않았을때 사용.
...
let copy02 = { ...user }; // {name : "seok", age : 40} 참조 복사가 아니라 포로퍼티를 펼쳐서 복사
copy02.age = 50;
...
//let age = user.age;
//let name = user.name;
let { age, name } = user; //위와 동일한 효과
let { age: username, namd } = user; //가져온 프로퍼티명 변경
...
/*
    단축 평가
        &&(and) : 앞의 값이 true일때 뒤의 값을 평가.
        ||(or) : 앞의 값이 false일때 뒤의 값을 평가.
*/
...
user.age > 30 && print(); // user.age가 30보다 클때만 print()를 평가(실행)

/* 
    null 병합 연산자(??)
        - null, undefined일 경우 실행
        - ?? 앞의 값이 null, undefined 일 경우 ?? 뒤의 값을 평가한다.
*/
0 || console.log("실행"); // 0은 falsy한 값이지만 null이나 undefined가 아니기 때문에 실행
...
//console.log(obj.weight.num); // 프로퍼티 값을 읽을 경우 에러 발생
...
/* 
    Array.prototype.filer() : 콜백함수의 return true인 요소만 모아서 새로운 배열로 반환.
        - return 값이 true, false여야 한다. => 조건식이면 된다.
*/
...
let resultArr01 = numbers.map((num, idx) => (idx % 2 !== 0 ? num * 2 : num));

console.log(resultArr01);
...
let filerList = userList.filter((user) => user.age >= 30);

console.log(filerList);

let nameList = filerList.map((user) => user.name);

console.log(nameList);

// 메서드 체이닝 : 메서드의 반환값에 따라 메서드를 이어서 실행한다.
let nameList01 = userList
  .filter((user) => user.age >= 30)
  .map((user) => user.name);

console.log(nameList01);
